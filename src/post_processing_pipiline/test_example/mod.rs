...
use crate::{ player::PlayerMarker, sys_paths };
...
pub struct TestExamplePostProcessPlugin;

impl Plugin for TestExamplePostProcessPlugin {
  ...
  fn finish(&self, app: &mut App) {
    ...
    render_app
      ...
      .init_resource::<SustomPostprocessPipeline>();
  }
}
...
impl ViewNode for CustomPostProcessNode {
  ...
  type ViewQuery = (
    &'static ViewTarget,
    // This makes sure the node only runs on cameras with the CustomPostProcessSettings component
    &'static CustomPostProcessSettings,
    // As there could be multiple post processing components sent to the GPU (one per camera),
    // we need to get the index of the one that is associated with the current view.
    &'static DynamicUniformIndex<CustomPostProcessSettings>,
  );
  ...
  fn run(
    ...
    let post_process_pipeline = world.resource::<SustomPostprocessPipeline>();
    ...
    let settings_uniforms = world.resource::<ComponentUniforms<CustomPostProcessSettings>>();
    ...
    let bind_group = render_context.render_device().create_bind_group(
      ...
      // It's important for this to match the BindGroupLayout defined in the SustomPostprocessPipeline
...
#[derive(Resource)]
struct SustomPostprocessPipeline {
  layout: BindGroupLayout,
  sampler: Sampler,
  pipeline_id: CachedRenderPipelineId,
}

impl FromWorld for SustomPostprocessPipeline {
  fn from_world(world: &mut World) -> Self {
    ...
    let shader = world.load_asset(
      sys_paths::shaders::post_processing_pipiline::EPostprocessingPipeline::CustomWaterExample.as_str()
    );
...
// prettier-ignore
fn update_settings(
  q_cam_pos: Query<&Transform, With<PlayerMarker>>,
  mut settings: Query<&mut CustomPostProcessSettings>, 
  time: Res<Time>,
) {
  ...
  if( trans.translation.y > 0.0 ){
    ...
    setting.cam_y = trans.translation.y.abs() / 50.0;
  }
