...
fn control_cam(
  ...
  mut query_camera: Query<&Transform, (With<CameraMarker>, Without<CameraParentMarker>)>,
  // mut query_camera_parent: Query<&mut Transform, (With<CameraParentMarker>, Without<CameraMarker>)>
  // mut query_camera_parent_2: Query<
  //   &mut Transform,
  ...
  let (entity, body, mut transform) = query_camera_parent.single_mut();
  ...
  const FORCE_SCALE_MUL: f32 = 100.0;
  ...
  if use_physics {
    if keys.pressed(KeyCode::KeyW) {
      let impulse3 = Vec3::new(x, y, z) * FORCE_SCALE_MUL * 10.0;
      let force = get_external_impulse(impulse3, false);
      commands.entity(entity).insert((RigidBody::Dynamic, force));
    } else if keys.pressed(KeyCode::KeyS) {
      let impulse3 = Vec3::new(x * -1.0, y * -1.0, z * -1.0) * FORCE_SCALE_MUL * 10.0;
      let force = get_external_impulse(impulse3, false);
      commands.entity(entity).insert((RigidBody::Dynamic, force));
    }
    ...
    if keys.pressed(KeyCode::KeyA) {
      let impulse3 = Vec3::new(x, y, z) * FORCE_SCALE_MUL;
      let force = get_external_impulse(impulse3, false);
      commands.entity(entity).insert((RigidBody::Dynamic, force));
    } else if keys.pressed(KeyCode::KeyD) {
      let impulse3 = Vec3::new(x * -1.0, y * -1.0, z * -1.0) * FORCE_SCALE_MUL;
      let force = get_external_impulse(impulse3, false);
      commands.entity(entity).insert((RigidBody::Dynamic, force));
    }
  } else {
